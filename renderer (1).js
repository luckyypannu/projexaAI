...
// Human-readable labels for each input type shown in result header
// Values match what backend returns in data.type field exactly
const typeLabels = {
  url:   'WEBSITE URL',
  phone: 'PHONE NUMBER',
  email: 'EMAIL ADDRESS',   // NEW — added for email support
};

/**
 * renderResult(data)
 * Main render function. Call this after successful API response.
 * Reads all fields using EXACT backend field names.
 *
 * @param {object} data — raw JSON from Flask /scan endpoint
 */
function renderResult(data) {

  // ── Read all backend fields by exact name ────────────────────
  const inputVal  = data.input         || '';
  const inputType = data.type          || 'url';    // "url" | "phone" | "email"
  const score     = data.trust_score   ?? 0;
  const riskLevel = data.risk_level    || 'High';   // "Low" | "Medium" | "High"
  // const flags     = data.pattern_flags || [];       // plain string array
  // const advice    = data.advice        || [];       // plain string array

  const flags     = data.pattern_flags || [];
  const advice    = data.advice        || [];
  // Optionally surface URLhaus threat if present and not already in flags
  ...
  // ── Result header ────────────────────────────────────────────
  // Shows: "EMAIL ADDRESS: support@gmai1.com"
  ...
  // ── Threat indicators (pattern_flags — plain string array) ───
  // Works for url, phone, AND email flags from backend
  ...
  // ── Advice (advice — plain string array) ─────────────────────
  // Works for url, phone, AND email advice from backend
